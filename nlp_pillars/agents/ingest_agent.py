...
# Optional LLM deps are not required for ingestion. Avoid importing heavy deps.
...
class IngestAgent:
    ...
    def __init__(self, model: Optional[str] = None, cache_dir: str = ".cache/pdfs"):
        ...
        # LLM client is not needed for ingestion; set agent to None to avoid heavy deps
        self.agent = None
    ...
    def reset_context(self):
        ...
        if getattr(self, "agent", None) and getattr(self.agent, "config", None):
            self.agent.config.history.clear()
            return
        # No-op when agent is not initialized
